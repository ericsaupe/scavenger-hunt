...
export default class extends Controller {
  ...
  showContent(event) {
    ...
    if (event.target.dataset.video === 'true') {
      content.innerHTML = `<video class="m-auto sm:flex-grow h-screen" controls loop src='${event.target.dataset.src}'></video>`
    } else {
      // Trying to load the image before showing it to stop the popping effect
      const img = new Image()
      img.src = event.target.dataset.src
      img.classList.add("m-auto", "max-h-screen", "object-scale-down")
